...
import { createCharacter, generateName, pickWeighted } from '../../domain/character.js';
...
export function CharacterForge({ onSave }) {
  ...
  const pickIndex = useCallback(() => {
    const option = pickWeighted(currentSlot.options, rngRef.current);
    return currentSlot.options.indexOf(option);
  }, [currentSlot]);
  ...
  const spinAll = useCallback(() => {
    ...
    const next = {};
    for (const slot of SLOTS) next[slot.id] = pickWeighted(slot.options, rng).id;
    setPicks(next);
