...
export class BattleSimulation {
  ...
  constructor({ characters, mode = BattleMode.FFA, teams = null, seed }) {
    ...
    /**
     * Objek `world` dibuat sekali dan dimutasi, bukan dibuat ulang tiap step.
  ...
  step(dt) {
    ...
    for (const f of this.list) {
      if (!isAlive(f)) continue;
      const target = computeSteering(f, this.fighters, this.rng, dt, this.world);
    ...
    // Batu diselesaikan SETELAH tumbukan antar bola. Urutannya penting: bola
    // yang baru terpental oleh lawan harus bisa langsung terhantam ke batu di
    // step yang sama, karena justru rangkaian itu yang ingin terasa —
    // dipukul, terlempar, membentur batu.
    resolveObstacleCollisions(this.list, this.obstacles, (f, o, impactSpeed) =>
      applyRockImpact(f, o, impactSpeed, this.ctx),
    );
